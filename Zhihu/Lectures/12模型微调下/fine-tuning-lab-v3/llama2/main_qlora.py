"""
LLaMa2是7B，一个4090跑不下，只能做QLoRA
主流程main()函数，加载模型、对模型做注入、加载数据集、训练、评估
...
def create_bnb_config():
    bnb_config = BitsAndBytesConfig(
        ...
        bnb_4bit_quant_type="nf4",  # 数据格式
        bnb_4bit_compute_dtype=torch.bfloat16,  # 存储方式
    )
...
def main():
    ...
    """
    model_args, data_args, peft_args, training_args = (
        parser.parse_args_into_dataclasses()
    )

    bnb_config = create_bnb_config()  # 将模型转为双重量化的形式
    ...
    if peft_args.lora_checkpoint is not None:
        model = load_qlora(
            raw_model, peft_args.lora_checkpoint, logger=logger, merge=False
        )
    ...
    # 这个数据集的处理函数是自己写的
    data_processor = Preprocessor(data_args=data_args, tokenizer=tokenizer)
    ...
    data_collator = DataCollatorForSeq2Seq(  # 数据规整器
        tokenizer,
        model=model,
    ...
    training_args.generation_max_length = (
        data_args.max_source_length + data_args.max_target_length + 1
    )
    training_args.generation_num_beams = 1

    trainer = QLoRATrainer(  # 训练器
        model=model,
        args=training_args,
