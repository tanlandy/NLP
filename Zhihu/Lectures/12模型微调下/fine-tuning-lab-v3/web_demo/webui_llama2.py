...
def get_completion(prompt):
    ...
    with torch.no_grad():
        outputs = model.generate(
            **inputs,
            max_length=max_target_length + max_source_length + 1,
            num_beams=1,
            do_sample=False
        )
...
def main():
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                search_field = gr.Textbox(
                    show_label=False, placeholder="search...", lines=8
                )
                user_input = gr.Textbox(
                    show_label=False, placeholder="输入框...", lines=2
                )
                with gr.Row():
        ...
        emptyBtn.click(
            reset_state,
            outputs=[chatbot, context, user_input, search_field, return_field],
        )
