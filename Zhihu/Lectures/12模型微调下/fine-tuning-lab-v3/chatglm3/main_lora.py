...
在模型加载load_lora_model(model_args, peft_args)稍微不一样，Lora是另一个参数注入的形式
...
def load_lora_model(model_args, peft_args):
    ...
    peft_config = LoraConfig(
        ...
        lora_dropout=peft_args.lora_dropout,  # dropout一般设置为0.1
        target_modules=["query_key_value"],  # 选择AB矩阵 要注入的模块
    )
