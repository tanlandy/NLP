...
官方的数据加载文件
定义了两个类：
MultiTurnDataset：多轮对话数据集
InputOutputDataset：单轮输入输出数据集
本质上都是一行一行加载，每一行加载完成之后，调用format_conversation函数，将对话转换成模型需要的格式。
format_conversation
...
def format_conversation(item, tokenizer, conversation_key: str, tool_key: str):
    ...
    # 用special token来区分从哪到哪是一轮
    # 用loss mask来标记是否需要计算loss
    ...
    if tool_key in item:
        conversations.insert(
            0,
            {
                ...
                + json.dumps(item[tool_key], indent=4, ensure_ascii=False),
            },
        )

    for idx, conv in enumerate(conversations):  # 对于每一轮
        loss = conv.get("loss", True)
        if conv["role"] in {"system", "user"}:  # 如果是系统或者用户，就不参与loss计算
            loss = False
        if conv["role"] == "tool":  # 如果是工具，就拼接成python的格式
            # function call python code
            value = (
                FUNCTION_CALL_PREFIX
                + format_function_call(FUNCTION_CALL_NAME, conv["parameters"])
                + FUNCTION_CALL_POSTFIX
            )
...
class MultiTurnDataset(Dataset):
    """多轮对话的形式，带Function calling的数据处理
    ...
    def __init__(
        self, data: List[dict], tokenizer: PreTrainedTokenizer, max_seq_length: int
    ):
    ...
    def __getitem__(self, i) -> dict:
        ...
        tokens, loss_masks = format_conversation(
            data_item, self.tokenizer, CONVERSATOIN_KEY, TOOL_DESC_KEY
        )
        ...
        assert len(tokens) == len(
            labels
        ), f"length mismatch: {len(tokens)} vs {len(labels)}"
