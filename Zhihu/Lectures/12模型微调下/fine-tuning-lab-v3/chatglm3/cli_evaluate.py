...
def load_pt2(model_args):
    ...
    prefix_state_dict = torch.load(
        os.path.join(model_args.checkpoint_path, "pytorch_model.bin")
    )
...
def load_lora(model_args, peft_args):
    ...
    if os.path.exists(os.path.join(model_args.checkpoint_path, "pytorch_model.bin")):
        model.load_state_dict(
            torch.load(os.path.join(model_args.checkpoint_path, "pytorch_model.bin")),
            strict=False,
        )
    model = model.to("cuda")
    return tokenizer, model
...
def chat(tokenizer, model, query, history, role):
    ...
    for response in response.split("<|assistant|>"):
        ...
        if not metadata.strip():
            ...
            history.append(
                {"role": "assistant", "metadata": metadata, "content": response}
            )
...
class Evaluator:
    ...
    def _bleu4(self, pred, label):
        ...
        bleu_score = sentence_bleu(
            [reference], hypothesis, smoothing_function=SmoothingFunction().method3
        )
        return bleu_score
    ...
    def _slot_accuracy(self, pred, label):
        ...
        pred_slots = (
            sum(len(v) if isinstance(v, list) else 1 for v in pred.values())
            if pred
            else 0
        )
        true_slots = (
            sum(len(v) if isinstance(v, list) else 1 for v in label.values())
            if label
            else 0
        )
    ...
    def evaluate(self):
            ...
            for turn in dialog:
                if turn["role"] == "user":
                    ...
                    response, history = chat(
                        self.tokenizer, self.model, turn["content"], history, "user"
                    )
                ...
                if turn["role"] == "tool":
                    ...
                    correct, pred_slots, true_slots = self._slot_accuracy(
                        pred_slot, label_slot
                    )
                    ...
                    if "observation" in turn:
                        ...
                        response, history = chat(
                            self.tokenizer,
                            self.model,
                            json.dumps(turn["observation"], ensure_ascii=False),
                            history,
                            "observation",
                        )
        ...
        score_dict["slot_P"] = (
            float(correct_slot_count / pred_slot_count) if pred_slot_count > 0 else 0
        )
        score_dict["slot_R"] = (
            float(correct_slot_count / true_slot_count) if true_slot_count > 0 else 0
        )
        score_dict["slot_F1"] = (
            2
            * score_dict["slot_P"]
            * score_dict["slot_R"]
            / (score_dict["slot_P"] + score_dict["slot_R"])
            if (score_dict["slot_P"] + score_dict["slot_R"]) > 0
            else 0
        )
