...
def handler(
    user_input, chatbot, history, prompt_field, tools_field, search_field, return_field
):
...
def reset_state():
    return (
        "",
        [],
        build_history(system_prompt, tool_description),
        system_prompt,
        tool_description,
        "",
        None,
...
def main():
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                user_input = gr.Textbox(
                    show_label=False, placeholder="输入框...", lines=1
                )
                with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                prompt_field = gr.Textbox(
                    show_label=False, interactive=True, value=system_prompt, lines=1
                )
                gr.HTML("""<h4>tool description</h4>""")
                tools_field = gr.Textbox(
                    show_label=False, interactive=True, value=tool_description, lines=4
                )
                gr.HTML("""<h4>function parameter</h4>""")
                search_field = gr.Textbox(
                    show_label=False, placeholder="搜索条件...", lines=6
                )
    ...
    demo.queue().launch(
        share=False, server_name="0.0.0.0", server_port=6006, inbrowser=True
    )
